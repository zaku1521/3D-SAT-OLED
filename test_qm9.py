...
from draw import draw_qm, init_qm9_draw
...
def get_root_logger(namespace='OLED(QSAR)-qm9', log_file=None, log_level=logging.INFO, file_mode='a'):
    """
...
class WR2Loss(torch.nn.Module):
    ...
    def forward(self, y_pred, y_true):
        W = torch.tensor([0.3, 0.3, 0.4]).reshape((3, 1)).to(y_true.device)
        # print(f"W:{W}")
        sse = torch.sum(torch.square(y_true - y_pred), dim=0)
        # print(f"sse:{sse}")
        sst = torch.sum(torch.square(y_true - torch.mean(y_true, dim=0)), dim=0)
        # print(f"sst:{sst}")
        loss = torch.einsum("ij,ji->i", (1 - sse / sst).reshape(1, 3), W) - 1
        return -loss
...
LOSS_RREGISTER = {
    ...
    'multilabel_regression': {
        ...
        'WR2Loss': WR2Loss(),
        'R2Loss': R2Loss(),
        'WeightedMSELoss': WeightedMSELoss(weights=[1.0, 1.0, 1.4]),
    }
}
ACTIVATION_FN = {
...
qm9_overall_metrics_dict = {}
qm9_per_target_metrics_dict = {}
...
class NNModel(object):
    ...
    def test(self):
        get_root_logger().info("start test QM9 BY 3D-SAT-OLED-Model:{}".format(self.model_name))
        ...
        val_indices = list(range(0, len(X)))
        ...
        _y_pred = self.trainer.val_predict(self.model, validdataset, self.loss_func,
                                           self.activation_fn, self.save_path, None, self.target_scaler)

        y_pred[val_indices] = _y_pred
        if 'multiclass_cnt' in self.data:
            label_cnt = self.data['multiclass_cnt']
        else:
            label_cnt = None

        y_valid_inv = self.data['target_scaler'].inverse_transform(y_valid)
        y_pred_inv = self.data['target_scaler'].inverse_transform(_y_pred)

        y_valid_ev = y_valid_inv * HARTREE_TO_EV
        y_pred_ev = y_pred_inv * HARTREE_TO_EV

        overall_metrics_hartree = self.metrics.cal_metric(
            y_valid_inv,
            y_pred_inv,
            label_cnt=label_cnt
        )
        get_root_logger().info(f"Overall result (unit: Hartree): {overall_metrics_hartree}")
        qm9_overall_metrics_dict['Hartree'] = overall_metrics_hartree
        overall_metrics_ev = self.metrics.cal_metric(
            y_valid_ev,
            y_pred_ev,
            label_cnt=label_cnt
        )
        get_root_logger().info(f"Overall result (unit: eV): {overall_metrics_ev}")
        qm9_overall_metrics_dict['eV'] = overall_metrics_ev
        ...
        for i in range(num_targets):
            ...
            per_target_metric_hartree = self.metrics.cal_metric(
                y_valid_inv[:, i].reshape(-1, 1),
                y_pred_inv[:, i].reshape(-1, 1),
                label_cnt=label_cnt
            )
            qm9_per_target_metrics_dict[(target_name, 'Hartree')] = per_target_metric_hartree
            get_root_logger().info(
                f"Result for target {i} ({target_name}) (unit: Hartree): {per_target_metric_hartree}")

            per_target_metric_ev = self.metrics.cal_metric(
                y_valid_ev[:, i].reshape(-1, 1),
                y_pred_ev[:, i].reshape(-1, 1),
                label_cnt=label_cnt
            )
            get_root_logger().info(f"Result for target {i} ({target_name}) (unit: eV): {per_target_metric_ev}")
            qm9_per_target_metrics_dict[(target_name, 'eV')] = per_target_metric_ev

        qm9_data = {
            'overall': qm9_overall_metrics_dict,
            'per_target': qm9_per_target_metrics_dict
        }
        qm9_data_converted = convert_tuple_keys(qm9_data)

        return qm9_data_converted
...
class Trainer(object):
    ...
    def val_predict(self, model, valid_dataset, loss_func, activation_fn, dump_dir, fold, target_scaler,
                    feature_name=None):
    ...
    def predict(self, model, dataset, loss_func, activation_fn, dump_dir, fold, target_scaler=None, epoch=1,
        ...
        model = model.to(self.device)
        if load_model == True:
            load_model_path = 'SAT_OLED_Model/weights/model_qm9.pth'
...
def collect_data(
    ...
    data_coords = [np.array(ast.literal_eval(item), dtype=np.float64) for item in data['coord']]
...
def main(cfg):
    ...
    exp_suffix = 'QM9_test' + '_' + exp_suffix
    ...
    qm9_metrics = clf.fit(data)

    metrics_path = os.path.join('./qm9_metrics', 'qm9_metrics.json')
    os.makedirs(os.path.dirname(metrics_path), exist_ok=True)
    with open(metrics_path, 'w', encoding='utf-8') as f:
        json.dump(qm9_metrics, f, ensure_ascii=False, indent=4)
        logger.info(f'Saved QM9 DataSet metrics to {metrics_path} ')

    logger.info(f'Start drawing QM9 dataset metrics, saving to: qm9_metrics')
    init_qm9_draw()
    draw_qm()
    return
