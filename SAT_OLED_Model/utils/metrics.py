# - Added new regression metrics: root_mean_squared_error (rmse), mean_absolute_percentage_error (mape), median_absolute_error, max_error
# - Added 'corr' metric (Pearson correlation) for multilabel regression tasks
# - Extended the 'multilabel_regression' section in METRICS_REGISTER with additional metrics (rmse, mape, median_ae, max_error, corr)
# - Updated DEFAULT_METRICS for 'multilabel_regression' to include rmse, mape, corr
# - Added import for median_absolute_error and max_error from sklearn.metrics
...
METRICS_REGISTER = {
    ...
    'multilabel_regression': {
        "mae": [mean_absolute_error, False, 'float'],
        "mse": [mean_squared_error, False, 'float'],
        "rmse": [root_mean_squared_error, False, 'float'],
        "r2": [r2_score, True, 'float'],
        "mape": [mean_absolute_percentage_error, False, 'float'],
        "median_ae": [median_absolute_error, False, 'float'],
        "max_error": [max_error, False, 'float'],
        "corr": [lambda y_true, y_pred: pearsonr(y_true, y_pred)[0], True, 'float'],
    }
}
...
DEFAULT_METRICS = {
    ...
    "multilabel_regression": ['mse', 'mae', 'r2', 'rmse', 'mape', 'corr'],
}
