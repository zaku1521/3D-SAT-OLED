...
class DyT(nn.LayerNorm):
    ...
    def __repr__(self):
        return f"DyT(({self.normalized_shape[0]},), eps={self.eps}, elementwise_affine={self.elementwise_affine})"
...
class SelfAttention(nn.Module):
    ...
    def forward(
        ...
        bsz, tgt_len, embed_dim, head_dim = query_layer.size()
        ...
        if attention_mask is not None:
            attention_mask = attention_mask.view(bsz, tgt_len, embed_dim, embed_dim)
...
class BertSelfOutput(nn.Module):
    def __init__(self, config):
        ...
        self.LayerNorm = DyT(config.hidden_size, eps=config.layer_norm_eps)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
...
class BertAttention(nn.Module):
    def __init__(self, config, position_embedding_type=None):
        ...
        self.attention = SelfAttention(config, position_embedding_type=position_embedding_type)
