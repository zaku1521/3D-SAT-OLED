...
# -----------------------------------------------------------------------------
# This file has been modified and extended based on the original version.
# The core architecture has been replaced by the SAT_OLED_Model, which incorporates MSE
# (MaterialStructureEncoder)—a customized BERT-based encoder tailored for OLED molecular modeling.
# MSE integrates geometry-aware self-attention by embedding positional encodings directly into
# the attention mechanism (QK interaction), enhancing spatial sensitivity.
# Additional components such as DyT (Dynamic Token-wise Normalization) and AngleExtractor
# are introduced to further capture molecular geometric features.
# These architectural upgrades enable accurate multi-property prediction for OLED molecules
# (PLQY, E_ad, HOMO, LUMO, GAP), transforming UniMol into a specialized framework for
# large-scale OLED property prediction and screening.
# -----------------------------------------------------------------------------
...
from .material_structure_encoder import MaterialStructureEncoder
from ..utils import pad_1d_tokens, pad_2d, pad_coords
import argparse
from ..data import Dictionary


class SAT_OLED_Model(nn.Module):
    """
       Attributes:
    ...
    def __init__(self, output_dim=3, data_type='molecule', **params):
        """
        Initializes the SAT_OLED_Model with specified parameters and data type.

        :param output_dim: (int) The number of output dimensions (classes).
        :param data_type: (str) The type of data (e.g., 'molecule', 'oled').
        ...
        self.gpe_proj = NonLinearHead(
            K, self.args.encoder_attention_heads, self.args.activation_fn
        )
        if self.args.kernel == 'gaussian':
            self.dpe = DistanceExtractor(K, n_edge_type)
        else:
            self.dpe = NumericalEmbed(K, n_edge_type)
        self.ape = AngleExtractor(num_kernels=64, hidden_dim=128, output_dim=64)
        self.classification_head = MaterialAttributePredictorHead(
        ...
        self.encoder = MaterialStructureEncoder()

    @classmethod
    def build_model(cls, args):
        """
        Class method to build a new instance of the SAT_OLED_Model.

        :param args: Arguments for model configuration.
        :return: An instance of SAT_OLED_Model.
        """
        return cls(args)
    ...
    def forward(
        ...
        def get_dist_features(dist, et):
            n_node = dist.size(-1)
            gbf_feature = self.dpe(dist, et)
            gbf_result = self.gpe_proj(gbf_feature)
        ...
        output = self.encoder(x, graph_attn_bias, padding_mask)[0]

        test_cls_repr = output[:, 0, :]

        test_logits = self.classification_head(test_cls_repr)
        return test_logits
...
class MaterialAttributePredictorHead(nn.Module):
...
class DistanceExtractor(nn.Module):
    """Attributes:
...
class AngleExtractor(nn.Module):

    def __init__(self, num_kernels=64, hidden_dim=128, output_dim=64):
        super(AngleExtractor, self).__init__()
        self.num_kernels = num_kernels
        self.mu = nn.Parameter(torch.linspace(0, np.pi, num_kernels).view(1, 1, 1, 1, num_kernels))
        self.sigma = nn.Parameter(torch.ones(1, 1, 1, 1, num_kernels) * 0.2)
        self.linear1 = nn.Linear(num_kernels, hidden_dim)
        self.linear2 = nn.Linear(hidden_dim, output_dim)

    def forward(self, theta, mask):
        theta = theta.unsqueeze(-1)  # [B, N, N, N, 1]
        gaussian = torch.exp(-((theta - self.mu) ** 2) / (2 * self.sigma.abs() ** 2 + 1e-8))  # [B, N, N, N, K]
        gaussian = gaussian * mask.unsqueeze(-1)
        agg = gaussian.sum(dim=(2, 3))  # [B, N, K]
        x = F.relu(self.linear1(agg))
